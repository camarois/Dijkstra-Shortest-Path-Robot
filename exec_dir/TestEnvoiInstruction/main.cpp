...
int main()
{
	...
	PORTA = VERT;*/
        ...
        uint8_t taille1 = receptionUART();
        uint8_t taille2 = receptionUART();
        
        uint16_t nbInstructions = ((taille1 << 8) | taille2 )- 0x02;
        Memoire24CXXX().ecriture(0x00, taille1);
        _delay_ms(5);
        Memoire24CXXX().ecriture(0x01, taille2);
        _delay_ms(5);
        
        if (nbInstructions == 34) PORTA =0x01;
        for(uint8_t j = 0; j < nbInstructions; j++)
	{
                uint8_t donnee = receptionUART();
		Memoire24CXXX().ecriture(0x02 + j, donnee);
                _delay_ms(5);
